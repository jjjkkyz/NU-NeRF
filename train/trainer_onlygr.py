...
from network.renderer_onlygr import name2renderer
    ...
    def run(self):
        ...
        # with torch.autograd.detect_anomaly():
            ...
            #for name, param in self.network.named_parameters():
                # if name.find('light') != -1 and name.find('stage1') != -1:# and name.find('sdf') == -1:
                #     if param.grad is not None:
                #         param.grad.zero_()
                # if name.find('occ') != -1 and name.find('stage1') != -1:# and name.find('sdf') == -1:
                #     if param.grad is not None:
                #         param.grad.zero_()
                #if name.find('stage1') != -1 and name.find('outer_nerf') == -1:
                #    if param.grad is not None:
                #        param.grad.zero_()
