...
class Trainer:
    ...
    def run(self):
        ...
        # checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/refrac_withexpref_bunny_gazebo_allview_srgb1/model_best.pth')
        ...
        # self.network.color_network.dir_enc1, dir_dim = get_embedder(12, 3)
        # self.network.color_network.pos_enc1, pos_dim = get_embedder(10, 3)
        # self.network.color_network.refrac_light1 = make_predictor1(pos_dim + dir_dim + 256, 3, activation='exp')
        ...
        for step in range(start_step, self.cfg['total_step']):
           ...
           #print(self.network.color_network.refrac_light1[0].grad)
            ...
            if True or step == 0 or (step + 1) % self.cfg['val_interval'] == 0 or (step + 1) == self.cfg['total_step']:
