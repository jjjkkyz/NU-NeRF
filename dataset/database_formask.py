...
class NeRFSyntheticDatabase(BaseDatabase):
    def __init__(self, database_name, dataset_dir, testskip=8):
        ...
        splits = ['train', 'test']
        meta = {}
        with open(os.path.join(self.root, 'transforms.json'), 'r') as fp:
            meta = json.load(fp)
        ...
        for frame in meta['frames']:
            ...
            fname_real = frame['file_path'].split('/')[-1]
            fname_path = frame['file_path'][:-len(fname_real)]
            fname_mask = fname
            #fname_mask = os.path.join(fname_path + 'mask_blurred/' + fname_real + '.jpg')
