...
from network.field import SDFNetwork, SingleVarianceNetwork, NeRFNetwork, AppShadingNetwork, get_intersection, \
    extract_geometry, sample_pdf, MCShadingNetwork, AppShadingNetwork_S2, InfOutNetwork, IoRNetwork, ThicknessNetwork
...
class Stage2Renderer(nn.Module):
    ...
    def __init__(self, cfg, training=True):
        ...
        checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/refrac_thickbottle_change/model.pth')
       
        self.stage1_network = NeROShapeRenderer(cfg,training=False)
       ...
       # self.infinity_far_bkgr = InfOutNetwork()
        ...
        mesh_path = '/home/sunjiamu/NeRO/data/meshes/refrac_thickbottle_change-260000_invert.ply'
    ...
    def ray_trace(self,rays_o,rays_d):
        ...
        # print((normal * rays_d).sum(dim=-1))
         ...
         #   bounding_box_index = torch.ones(intersection.shape[0],device='cuda:0').int() * 0
           ...
           # print(bounding_box_index)
            ...
            ior_ratio = 1 / (ior_ratio * 1 + 0.8)

            ior_ratio = 1 / (torch.ones_like(ior_ratio) * 1.0)
            ior_ratio = torch.clamp(ior_ratio, max= torch.ones_like(ior_ratio,device='cuda:0'))

            ior_inner_ratio = self.IoRint_pred(torch.zeros_like(intersection.reshape(-1,3),device='cuda:0')).reshape(-1,1)
           # ior_inner_ratio = 1 / (ior_inner_ratio * 1 + 0.8)

            ior_inner_ratio = 1 / (torch.ones_like(ior_inner_ratio) * 1.5)
            ior_inner_ratio = torch.clamp(ior_inner_ratio, max= torch.ones_like(ior_inner_ratio,device='cuda:0'))

            thickness = self.thickness_pred(torch.zeros_like(intersection.reshape(-1,3),device='cuda:0')).reshape(-1,1)
            #thickness = thickness * 0.08
            thickness = torch.ones_like(thickness) * 0.05
         ...
         #   print(converged_out.flatten())
            ...
            if is_outside:
                next_dir_inner = (ior_inner_ratio * next_dir[converged_out.flatten()] + (ior_inner_ratio * cos_thetai[converged_out[mask_converge].flatten()] - torch.sqrt(1 - sin_thetat_2_inner)) * normal[converged_out[mask_converge].flatten()])

                thickness = thickness[converged_out[mask_converge].flatten()].flatten()
                ...
                print(discrete_gaussian_curvature_measure(self.trimesh_mesh,intersection[converged_out[mask_converge].flatten()].reshape(-1,3)))
                exit(1)
            
                next_start_tmp = intersection[converged_out[mask_converge].flatten()] + next_dir_inner.reshape(-1,3) * (thickness.reshape(-1,1) + 0.003) / (torch.linalg.norm(next_dir_inner,dim=-1).reshape(-1,1) * cos_thetat_inner.reshape(-1,1))
            #  print(torch.linalg.norm(next_dir_tmp,dim=-1))
                ...
                intersection[converged_out[mask_converge].flatten()] = intersection[converged_out[mask_converge].flatten()] - next_dir[converged_out.flatten()].reshape(-1,3) * thickness.reshape(-1,1) / (torch.linalg.norm(next_dir[converged_out.flatten()],dim=-1).reshape(-1,1) * cos_thetai[converged_out[mask_converge].flatten()].reshape(-1,1))

                #print(sin_thetat_2.max())
                next_dir_inner = (ior_inner_ratio * next_dir[converged_out.flatten()] + (ior_inner_ratio * cos_thetai[converged_out[mask_converge].flatten()] - torch.sqrt(1 - sin_thetat_2_inner)) * normal[converged_out[mask_converge].flatten()])
      ...
      #  print(len(sampled_pathes))
       ...
       # pc = trimesh.PointCloud(vertices = vert.detach().cpu().numpy())
       # trimesh.exchange.export.export_mesh(pc,'tst_pc_optixt.ply')
       # exit(1)
         ...
         #   print(current_transmission_portion)
          ...
          #  print(i)
           ...
           # print(converges[i])
            ...
            # if i == 1:
            #     alpha_nerf[inner_mask], gradients, feature_vector, inv_s, sdf = self.compute_sdf_alpha(points_for_nerf[inner_mask],
            #                                                                                     dists[inner_mask],
            #                                                                                     dirs[inner_mask],
            #                                                                                     cos_anneal_ratio, step)
            ...
            #     sampled_color_nerf[inner_mask], occ_info = self.color_network_inner(points_for_nerf[inner_mask], gradients, -dirs[inner_mask],
