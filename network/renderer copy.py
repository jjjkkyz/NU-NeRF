...
class Stage2Renderer(nn.Module):
    ...
    def __init__(self, cfg, training=True):
        ...
        checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/ball_0_7_0_68/model.pth')       
        cfg1 = cfg
        cfg1['shader_config']['sphere_direction'] = False
       ...
       # self.infinity_far_bkgr = InfOutNetwork()
        ...
        mesh_path = '/home/sunjiamu/NeRO/data/meshes/ball_0_7_0_68-95000_inverted.ply'
        ...
        # print((normal * rays_d).sum(dim=-1))
         ...
         #   bounding_box_index = torch.ones(intersection.shape[0],device='cuda:0').int() * 0
           ...
           # print(bounding_box_index)
            ...
            ior_ratio = 1 / 1.45 + 0 * 1 / (ior_ratio * 0.4 + 1.0) #* 0 +  1 / 1.115
            ...
            ior_inner_ratio = 0.9999 + 0 * 1 / (ior_inner_ratio * 0.8 + 1.0)
            ...
            ior_ratio_anotherside = ior_ratio / ior_inner_ratio 

            thickness = self.thickness_pred(intersection.reshape(-1,3)).reshape(-1,1)
            thickness = thickness * 0.04 * 0 + 0.015
         ...
         #   ior_ratio = torch.where(bounding_box_index == 9, 1, 1/1.5).reshape(-1,1)
          ...
          #  print(ior_ratio * ior_ratio * sin_thetai_2)
            if not is_outside: 
                ...
                ior_ratio_anotherside = 1 / ior_inner_ratio
                ior_inner_ratio = ior_inner_ratio * ior_ratio
         ...
         #   print(ior_ratio * ior_ratio * sin_thetai_2)
           ...
           # converged_out
            ...
            #converged_out[mask_converge] *= torch.where(ior_inner_ratio * ior_inner_ratio * sin_thetai_2 > 0.999, False, True)
          ...
          #  ior_ratio_anotherside = ior_ratio_anotherside[converged_out[mask_converge].flatten()]
            ...
            curvature_radius = 1 / torch.sqrt(torch.abs(gaussian_curvature))
            

            
            if is_outside:
                cos_thetat = torch.sqrt(1 - sin_thetat_2)
                ...
                next_dir_inner = (ior_ratio * next_dir[converged_out.flatten()] + (ior_ratio * cos_thetai[converged_out[mask_converge].flatten()] - torch.sqrt(1 - sin_thetat_2)) * normal[converged_out[mask_converge].flatten()])
                ...
                if curvature_positive.sum() > 0:
                    costhetat_r = curvature_radius * cos_thetat_inner
                    ...
                    next_start_tmp_positive = intersection[converged_out[mask_converge].flatten()] + next_dir_inner.reshape(-1,3) * (length_dt.reshape(-1,1) + 0.001) / (torch.linalg.norm(next_dir_inner,dim=-1).reshape(-1,1) * cos_thetat_inner.reshape(-1,1))
                ...
                if curvature_negative.sum() > 0:
                    ...
                    delta_negative = torch.sqrt(torch.clamp(delta2_negative,min = 0))
                    length_dt = torch.abs(costhetat_r - delta_negative)
                   # print(11,length_dt.min())

                    center_position = intersection[converged_out[mask_converge].flatten()] + normal[converged_out[mask_converge].flatten()] * curvature_radius
                    next_start_tmp_negative = intersection[converged_out[mask_converge].flatten()] + next_dir_inner.reshape(-1,3) * (length_dt.reshape(-1,1) + 0.001) / (torch.linalg.norm(next_dir_inner,dim=-1).reshape(-1,1) * cos_thetat_inner.reshape(-1,1))
               ...
               #print(next_dir_inner.shape)
                ...
                next_dir_tmp = (ior_ratio_anotherside * next_dir_inner + (ior_ratio_anotherside * cos_thetai_anotherside - torch.sqrt(1 - sin_thetat_2_anotherside)) * normal_after)
                next_dir_tmp = next_dir_tmp / (torch.linalg.norm(next_dir_tmp,dim=-1,keepdim=True) + 0.0001)
                
                
            else:
                cos_thetat_inner = torch.sqrt(1 - sin_thetat_2_inner)
                curvature_positive = gaussian_curvature <= 0
                curvature_negative = ~curvature_positive
                curvature_positive = curvature_positive.flatten()
                curvature_negative = curvature_negative.flatten()
                ...
                if curvature_positive.sum() > 0:
                    ...
                    delta_positive = torch.sqrt(torch.clamp(delta2_positive,min = 0))
                ...
                sin_thetat_2_inner_modified = torch.clamp(sin_thetai_2_modified[converged_out[mask_converge].flatten()] * ior_inner_ratio * ior_inner_ratio, max= 0.999)

                tir[converged_out.flatten()] *= torch.where(sin_thetai_2_modified[converged_out[mask_converge].flatten()] * ior_inner_ratio * ior_inner_ratio > 0.999, False, True).detach()

                next_dir_inner = (ior_inner_ratio * next_dir[converged_out.flatten()] + (ior_inner_ratio * cos_thetai_modified[converged_out[mask_converge].flatten()] - torch.sqrt(1 - sin_thetat_2_inner_modified)) * normal_modified[converged_out[mask_converge].flatten()])
                next_dir_inner = next_dir_inner / (torch.linalg.norm(next_dir_inner,dim=-1,keepdim=True) + 0.0001)
                ...
                if curvature_positive.sum() > 0:
                    costhetat_r = curvature_radius * cos_thetat_inner
                    delta2_positive = costhetat_r * costhetat_r - 2 * curvature_radius * thickness + thickness * thickness
                    delta_positive = torch.sqrt(torch.clamp(delta2_positive,min = 0.001))
                    length_dt = torch.abs(costhetat_r - delta_positive)
          ...
          #  print(converged_tmp.flatten().shape)
           ...
           # print(k)
            ...
            if not torch.all(~infinity_bkgr[k]):
                ...
                # z_vals_outside = lower[None, :] + (upper - lower)[None, :] * t_rand
               # if k != 2:
                z_vals_outside = 6.0 / torch.flip(z_vals_outside, dims=[-1]) + 1.0 / 64
               # else:
             ...
             #   print(outer_mask.sum())
               ...
               # trimesh.exchange.export.export_mesh(pc,'outerx.ply')
                ...
                sampled_color_nerf[inner_mask], occ_info = self.color_network_inner(points_for_nerf[inner_mask], gradients_inner, -dirs[inner_mask],
                                                                        ...
                                                                        step=step)
                alpha_nerf = alpha_nerf.reshape(batch_size, n_samples) * 0
                sampled_color_nerf = sampled_color_nerf.reshape(batch_size, n_samples,3) * 0
