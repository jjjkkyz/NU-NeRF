...
from network.field_single import SDFNetwork, SingleVarianceNetwork, NeRFNetwork, AppShadingNetwork, get_intersection, \
...
class Stage2Renderer(nn.Module):
    ...
    def __init__(self, cfg, training=True):
        ...
        checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/refrac_kirsch_final2/model_best.pth')
       ...
       # self.infinity_far_bkgr = InfOutNetwork()
        ...
        mesh_path = '/home/sunjiamu/NeRO/data/meshes/refrac_kirsch_final2-195000_sim.ply'
        ...
        # print((normal * rays_d).sum(dim=-1))
         ...
         #   bounding_box_index = torch.ones(intersection.shape[0],device='cuda:0').int() * 0
           ...
           # print(bounding_box_index)
            ...
            ior_ratio = 1 / (ior_ratio * 0.8 + 1) * 0 + 1 / 1.001
         ...
         #   print(ior_ratio * ior_ratio * sin_thetai_2)
           ...
           # converged_out
            ...
            converged_out[mask_converge] = torch.where(ior_ratio * ior_ratio * sin_thetai_2 > 0.9999, False, True)
            tir[mask_converge] = torch.where(ior_ratio * ior_ratio * sin_thetai_2 > 0.9999, False, True).detach()
