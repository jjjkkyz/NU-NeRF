...
def process_key_img(data, h, w, k):
    ...
    cv2.putText(img, k, (0,30), font, font_scale, (255, 255, 255), thickness)
...
def get_key_images(data_pr, keys, h, w):
    results = []
    for k in keys:
        if k in data_pr: results.append(process_key_img(data_pr[k], h, w, k))
    return results
...
def draw_materials_s2(data_pr, h, w):
    ...
    keys = ['specular_light', 'specular_color', 'specular_ref',
        #'diffuse_albedo', 'diffuse_light', 'diffuse_color', 'refraction_light',
            #'specular_albedo',
           # 'metallic', 'roughness', 'occ_prob', 'indirect_light'
            ]
    results = get_key_images(data_pr, keys, h, w)
    results = [concat_images_list(*results[0:3])]
    return results

class Stage2RenderMetrics(Loss):
    def __init__(self, cfg):
        pass
    ...
    def __call__(self, data_pr, data_gt, step, **kwargs):
        ...
        imgs += draw_materials_s2(data_pr, h, w)
