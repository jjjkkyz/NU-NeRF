...
from network.field_onlygr import SDFNetwork, SingleVarianceNetwork, NeRFNetwork, AppShadingNetwork, get_intersection, \
...
class Stage2Renderer(nn.Module):
    ...
    def __init__(self, cfg, training=True):
        ...
        checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/realbottle3_limitlowfreq/model.pth')
       ...
       # self.infinity_far_bkgr = InfOutNetwork()
        ...
        mesh_path = '/home/sunjiamu/NeRO/data/meshes/realbottle3_limitlowfreq-300000_invert.ply'
    ...
    @staticmethod
    def upsample_nerf(rays_o, rays_d, z_vals, weights, n_importance):
        """
        Up sampling give a fixed inv_s
        """
        z_samples = sample_pdf(z_vals, weights, n_importance, det=True).detach()
         ...
         #   ior_ratio = torch.where(bounding_box_index == 9, 1, 1/1.5).reshape(-1,1)
          ...
          #  print(ior_ratio * ior_ratio * sin_thetai_2)
            if not is_outside: 
               ...
               # ior_inner_ratio = ior_ratio * ior_inner_ratio
        ...
        for k in range(len(converges)):
          ...
          #  print(converged_tmp.flatten().shape)
           ...
           # print(directions[i].shape)
            ...
            if k != 1:
                z_vals = torch.linspace(0, 1, 128).reshape(1,-1)
                sampled_vertices = start.reshape(-1,1,3) + (end.reshape(-1,1,3) - start.reshape(-1,1,3)) * z_vals.reshape(-1,128,1) 
            else:
           ...
           # print(k)
            if torch.any(~infinity_bkgr[k]):
                ...
                if k != 1:
                    sampled_vertices[~infinity_bkgr[k].flatten()] = start[~infinity_bkgr[k].flatten()].reshape(-1,1,3) + (end[~infinity_bkgr[k].flatten()].reshape(-1,1,3) - start[~infinity_bkgr[k].flatten()].reshape(-1,1,3)) * z_vals.reshape(-1,128,1)
                else:
            ...
            if not torch.all(~infinity_bkgr[k]):
                z_vals_outside = torch.linspace(1e-3, 1.0 - 1.0 / (128 + 1.0), 128)
              #  z_vals_outside = torch.linspace(0.01, 15.0, 64)
                ...
                # z_vals_outside = lower[None, :] + (upper - lower)[None, :] * t_rand
#4.5
                z_vals_outside = 6.0 / torch.flip(z_vals_outside, dims=[-1]) + 1.0 / 128

                #print(z_vals_outside)
                

                sampled_vertices[infinity_bkgr[k].flatten()] = start[infinity_bkgr[k].flatten()].reshape(-1,1,3) + directions[k][infinity_bkgr[k].flatten()].reshape(-1,1,3) * z_vals_outside.reshape(-1,128,1)
            #print(k)
            #if k == 1: 
         ...
         #   print(current_transmission_portion)
          ...
          #  print(i)
           ...
           # print(converges[i])
            ...
            if i == 1:
                alpha_nerf, gradients_inner, feature_vector, inv_s_inner, sdf = self.compute_sdf_alpha(points_for_nerf[inner_mask],
                                                                                                dists[inner_mask],
                                                                                                dirs[inner_mask],
                                                                                                cos_anneal_ratio, step)
             ...
             #   print(outer_mask.sum())
                ...
                #exit(1)
                sampled_color_nerf, occ_info = self.color_network_inner(points_for_nerf[inner_mask], gradients_inner, -dirs[inner_mask],
            ...
            #print(feature_vector.shape)
               ...
               # print(directions[i+1].shape)
                ...
                if i == 0 and not is_train:      
                   ...
                   # iors = self.IORs_pred(points_for_neus.reshape(-1,3)) * 0.8 + 1
                   # iors = torch.clamp(iors,1.35,1.55)
                   # iors = (iors - 1.35) / 0.2
                    # torch.tensor(cm(iors.flatten().detach().cpu().numpy())[...,:3],device='cuda:0').float()
