...
from network.field_wogr import SDFNetwork, SingleVarianceNetwork, NeRFNetwork, AppShadingNetwork, get_intersection, \
...
class Stage2Renderer(nn.Module):
    ...
    def __init__(self, cfg, training=True):
        ...
        checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/pen1/model.pth')
       ...
       # self.infinity_far_bkgr = InfOutNetwork()
        ...
        mesh_path = '/home/sunjiamu/NeRO/data/meshes/pen1-300000_invert1.ply'
    ...
    def ray_trace(self,rays_o,rays_d):
       ...
       # inside_tracing_sdf = lambda x: -self.stage1_network.sdf_network.sdf(x)
        ...
        for i in range(3):
           ...
           # next_dir = torch.nan_to_num(next_start, 1.0)
            ...
            ior_ratio = 1 / (ior_ratio * 0.8 + 1)  #* 0 + 1 / 1.4
          ...
          #  print(converged_tmp.flatten().shape)
           ...
           # print(k)
            ...
            if not torch.all(~infinity_bkgr[k]) and k != 1:
                ...
                if k == 0:
                    z_vals_outside = torch.linspace(2.0, 15.0, 192)
                else:
                    z_vals_outside = torch.linspace(0.1, 12.0, 192)
