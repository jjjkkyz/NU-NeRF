...
class AppShadingNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.inner_light = make_predictor(pos_dim + 72, 3, activation='exp', exp_max=0.0)
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, human_poses, inter_results=False, step=None):
      ...
      #  ior = self.iors(torch.cat([feature_vectors, points], -1))
        ...
        #exit(1)
        # diffuse light
        diffuse_albedo = (1 - metallic) * albedo * (1 - transmission_weight)
     ...
     #   transmission_specular = transmission_specular * transmission_specular
      ...
      #  ref1_ = self.dir_enc1(view_dirs)
       # pts1 = self.pos_enc1(points)
        ...
        refraction_light = self.refrac_light(torch.cat([feature_vectors,pts, ref_], -1))
        ...
        # integrated together
        color = (diffuse_color + specular_color) + (1 - reflection_weight) * refraction_light * (1 - metallic) * transmission_weight
     ...
     #   color = torch.clamp(color, min=0.0, max=1.0)
        ...
        if inter_results:
            intermediate_results = {
                ...
                'refraction_light': torch.clamp(linear_to_srgb((1 - reflection_weight) * refraction_light * (1 - metallic) * transmission_weight), min=0.0, max=1.0),
...
class AppShadingNetwork_S2(nn.Module):
    ...
    def __init__(self, cfg, stage1):
     ...
     #   nn.init.constant_(self.refrac_weight[-2].bias, self.cfg['inner_init'])
        ...
        self.refrac_light = make_predictor(pos_dim + dir_dim, 3, activation='exp')
        nn.init.constant_(self.refrac_light[-2].bias, np.log(0.5))
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, ior_ratios, is_internal,next_dir, inter_results=False, step=None):
       ...
       # print(ior_ratios.shape)
        next_dir = next_dir
        normals = F.normalize(normals, dim=-1).detach()
        view_dirs = F.normalize(view_dirs, dim=-1)
        reflective = torch.sum(view_dirs * normals, -1, keepdim=True) * normals * 2 - view_dirs
        NoV = torch.sum(normals * view_dirs, -1, keepdim=True)
       # print(NoV.min())
      ...
      #  roughness = self.roughness_predictor(torch.cat([feature_vectors, points], -1))
      #  albedo = self.albedo_predictor(torch.cat([feature_vectors, points], -1))
        metallic = self.stage1_network.color_network.metallic_predictor(torch.cat([feature_vectors, points], -1)) * 0
     ...
     #   refrac_weight_pred = self.refrac_weight(torch.cat([feature_vectors, points], -1))
      #  refrac_weight_pred = self.refrac_weight(pts)
        #refrac_weight_pred = refrac_weight_pred * 0.5 + 0.5
      #  refrac_weight_pred = torch.clamp(refrac_weight_pred, min=0, max=1)
       # print(refrac_weight_pred)
        
        diffuse_albedo = albedo * (1 - transmission_weight) * (1 - metallic) 
        diffuse_light = self.predict_diffuse_lights(points, feature_vectors, normals)
        diffuse_color = diffuse_albedo * diffuse_light
      ...
      #  dialetric_albedo = (1 - ior_ratios) / (1 + ior_ratios)
      #  dialetric_albedo = dialetric_albedo * dialetric_albedo
       # print(ior_ratios)
       # print(dialetric_albedo)
        specular_albedo = 0.04 * (1 - metallic) + metallic * albedo
        specular_light, occ_prob, indirect_light, human_light = self.predict_specular_lights(points, feature_vectors,
                                                                                             reflective, roughness,
                                                                                             step)
        
        refraction_albedo = 0.04
        NoV_for_schlick = NoV
        if is_internal:
            NoV_for_schlick = torch.clamp(torch.sum(-normals * next_dir, -1, keepdim=True),min=0)
          #  print(torch.sum(-normals * next_dir, -1, keepdim=True).min())
           # print(NoV_for_schlick.max())
           # print(NoV_for_schlick)
         #   exit(1)
        #print(NoV_for_schlick)
        #print(refrac_weight_pred)
       # print(NoV_for_schlick.shape)
        temp_nov = torch.clamp(1 - NoV, min=0.0, max=1.0)
        schlick = refraction_albedo + (1 - refraction_albedo) * temp_nov * temp_nov * temp_nov * temp_nov * temp_nov
        reflection_weight = torch.clamp(schlick, min=0, max=1)
        # if reflection_weight.shape[1] > 1:
        #     print(refraction_albedo.shape)
        #     print(temp_nov.shape)
        #     print(reflection_weight.shape)
        #     exit(1)
        
        refraction_light = self.refrac_light(torch.cat([pts.detach(), ref_.detach()], -1))

        fg_uv = torch.cat([torch.clamp(NoV, min=0.0, max=1.0), torch.clamp(roughness, min=0.0, max=1.0)], -1)
        pn, bn = points.shape[0], 1
        fg_lookup = dr.texture(self.FG_LUT, fg_uv.reshape(1, pn // bn, bn, -1).contiguous(), filter_mode='linear',
                               boundary_mode='clamp').reshape(pn, 2)
        specular_ref = (specular_albedo * fg_lookup[:, 0:1].reshape(-1,1,1) + fg_lookup[:, 1:2].reshape(-1,1,1))
        specular_color = specular_ref * specular_light
       # print(specular_color.shape)
       # print(specular_light.shape)
        # integrated together
       ...
       # color = linear_to_srgb(color)
        #color = torch.clamp(color, min=0.0, max=1.0)
        if is_internal: color *= 0
  #      print(refraction_albedo)
   ...
   #     print(metallic)
     ...
     #   reflection_weight *= 0
        ...
        occ_info = {
            'reflective': reflective,
            'occ_prob': occ_prob,
            'refraction_coefficient': (1 - reflection_weight) * transmission_weight * (1 - metallic)
        }
        ...
        if inter_results:
            intermediate_results = {
                ...
                'diffuse_light': torch.clamp(linear_to_srgb(specular_light), min=0.0, max=1.0),
                'diffuse_color': torch.clamp(specular_color, min=0.0, max=1.0),
                ...
                'occ_prob': torch.clamp(occ_prob, max=1.0, min=0.0),
                'indirect_light': indirect_light,
                'refraction_light': refraction_light,
                'reflection_weight': reflection_weight,
            }
           #print(intermediate_results['refraction_light'])
           # exit(1)
