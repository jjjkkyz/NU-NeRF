...
from .tracing_optix import optix_mesh
import trimesh
import numpy as np
import imageio
...
@torch.jit.script
def JIT_Dintersect(origin:torch.Tensor, ray_dir:torch.Tensor, triangles:torch.Tensor, normals:torch.Tensor,curvatures:torch.Tensor):
    '''
   ...
   # n = n / n.norm(dim=1, p=2, keepdim=True).detach()
    ...
    k1 = curvatures[:,1]
    k2 = curvatures[:,2]
    g_k = (1-u-v).reshape((-1,1)) * k0 + u.reshape((-1,1)) * k1 + v.reshape((-1, 1)) * k2
...
class primary_edge_sample(torch.autograd.Function):
    ...
    def forward(ctx, E_pos, intersect_fun, camera_M, ray_origin):
        ...
        f_point = torch.cat((fu_point,fl_point), dim=0) #[2nx2]
        W = -torch.ones([f_point.shape[0],1], dtype=Float, device=device)
        camera_p = torch.cat([f_point, W], dim=-1).float() 
        camera_p[:,0] = (camera_p[:,0] - K[0][2]) / K[0][0]
        camera_p[:,1] =  -(camera_p[:,1] - K[1][2]) / K[1][1]
        camera_p = camera_p @ R[:3,:3].T
        #camera_p = torch.cat([camera_p, W], dim=0)
        #world_p = R_inverse @ camera_p #[4x2n]
        world_p = camera_p + R[:3,3:4].reshape(1,3).expand(camera_p.shape[0],3) #[2nx3]

       # pc = trimesh.PointCloud(vertices = world_p.detach().cpu().numpy())
       # trimesh.exchange.export.export_mesh(pc,'pc_camera_world2.ply')
       # exit(1)
        #world_p = world_p[:3].T #[2nx3]
        ray_dir = world_p - ray_origin.view(-1,3)
        ray_dir = torch.nan_to_num(ray_dir,0.0)
       ...
       # pc = trimesh.PointCloud(vertices = index_3d.detach().cpu().numpy())
       # trimesh.exchange.export.export_mesh(pc,'pc_index.ply')
        # debug
...
class Ray:
    ...
    def select(self, mask):
        return Ray(self.origin[mask],self.direction[mask])

    def __len__(self):
        return len(self.origin)

class Intersection:
    def __init__(self, u, v, t, n,g_k,ray:Ray, faces_ind):
        self.u = u
        self.v = v
        self.t = t
        self.n = n
        self.g_k = g_k
        self.intersection_point = ray.origin + t.reshape(-1,1) * ray.direction
        self.ray = ray
...
class Scene:
    ...
    def init_VN(self):
        ...
        self.gaussian_curvatures = torch.tensor(np.clip(self.pymesh_mesh.get_attribute("vertex_gaussian_curvature"),a_min=-10.0,a_max=10.0),device='cuda:0').reshape(vertices.shape[0],1)
    ...
    def primary_visibility(self, silhouette_edge, camera_M, origin, detach_depth = False):
        ...
        R,K = camera_M
        V = self.vertices[silhouette_edge.view(-1)].float() #[2Nx3]
        W = torch.ones([V.shape[0],1], dtype=Float, device=device)
        #v_hemo = torch.cat([V, W], dim=1) #[2Nx4]
        #v_camera =  R @ v_hemo.T #[4x2N]
       # print(V[:10])
        v_tmp = V - R[:3,3:4].reshape(1,3).expand(V.shape[0],3)
       # print(v_tmp[:10])
        v_camera =  v_tmp @ R[:3,:3]
        ...
        v_camera = v_camera / (-v_camera[:,2:3]).expand(V.shape[0],3)
        #print(v_camera[:10])
        v_camera[:,0] = v_camera[:,0] * K[0,0] + K[0,2]
        v_camera[:,1] = (-v_camera[:,1] * K[1,1] + K[1,2])
       # print(v_camera[:10])
       # pc = trimesh.PointCloud(vertices = V.detach().cpu().numpy())
       # trimesh.exchange.export.export_mesh(pc,'pc_camera_world1.ply')

       # pc = trimesh.PointCloud(vertices = v_camera.detach().cpu().numpy())
       # trimesh.exchange.export.export_mesh(pc,'pc_camera.ply')
      #  print(v_camera[:,0].max(),v_camera[:,0].min())
       # print(v_camera[:,1].max(),v_camera[:,1].min())
       # exit(1)
        pixel_index = v_camera[:,:2].T #[2x2N]
    ...
    def Dintersect(self, ray: Ray):
        ...
        curvatures = self.gaussian_curvatures[faces].float()
        ...
        return Intersection(u=u, v=v, t=t, n=n, g_k = g_k, ray=ray_hitted, faces_ind=faces_ind[hitted]), hitted
