...
from network.field import SDFNetwork, SingleVarianceNetwork, NeRFNetwork, AppShadingNetwork, get_intersection, \
    extract_geometry, sample_pdf, MCShadingNetwork, AppShadingNetwork_S2, InfOutNetwork
...
class NeROShapeRenderer(nn.Module):
    ...
    def _process_nerf_ray_batch(self, ray_batch, poses):
        ...
        rays_d = F.normalize(rays_d, dim=-1)
        near, far = torch.full((rays_o.shape[0], 1),  0.1), torch.full((rays_o.shape[0], 1), 8.0)
...
class Stage2Renderer(nn.Module):
    ...
    def __init__(self, cfg, training=True):
        ...
        iors = torch.ones(10,device='cuda:0') / 1.5
        self.IORs = torch.nn.Parameter(iors,requires_grad=True)
        self.register_parameter(name="IORs", param=self.IORs)

        self.infinity_far_bkgr = InfOutNetwork()
        checkpoint = torch.load('/home/sunjiamu/NeRO/data/model/refrac_withexpref_complex1/model_best.pth')
        self.stage1_network = NeROShapeRenderer(cfg)
        ...
        self.scene = Scene('/home/sunjiamu/NeRO/data/meshes/refrac_withexpref_complex1-300000.ply')
       ...
       # self.outer_nerf = self.stage1_network.outer_nerf
        ...
        self.color_network = AppShadingNetwork_S2(self.cfg['shader_config'])
      #  self.sdf_inter_fun = lambda x: self.sdf_network.sdf(x)
    ...
    def test_step(self, index, step, ):
        ...
        trn = self.cfg['test_ray_num']
        outputs_keys = ['ray_rgb', 'gradient_error', 'normal']
        outputs_keys += [
            #'diffuse_albedo', 'diffuse_light', 'diffuse_color','refraction_light',
           # 'specular_albedo', 'specular_light', 'specular_color', 'specular_ref',
           # 'metallic', 'roughness', 'occ_prob', 'indirect_light', 'occ_prob_gt',
    ...
    def render(self, rays_o, rays_d, near, far, human_poses, perturb_overwrite=-1, cos_anneal_ratio=0.0, is_train=True,
        ...
        :return:
        """
        pathes, converges, directions,ior_ratios = self.ray_trace(rays_o,rays_d)
      
        #perturb = self.cfg['perturb']
       # if perturb_overwrite >= 0:
       #     perturb = perturb_overwrite
       # z_vals = self.sample_ray(rays_o, rays_d, near, far, perturb)
        ret = self.render_core(rays_o, rays_d, pathes, converges, directions,ior_ratios, human_poses, cos_anneal_ratio=cos_anneal_ratio, step=step,
                               is_train=is_train, is_nerf=is_nerf)
        return ret
    ...
    def ray_trace(self,rays_o,rays_d):
        ...
        for i in range(7):
            radius = 1 if i == 0 else None
            rays_for_intersection = Ray(next_start,next_dir)
            if is_outside:
                # intersection, converged = network.tracing.sphere_tracing(
                #     outside_tracing_sdf,
                #     next_start,
                #     next_dir,
                ...
                intersection = self.scene.Dintersect(rays_for_intersection)

                intersection[~converged.flatten()] = 0
                normal = self.stage1_network.sdf_network.gradient(intersection)
        ...
        # print((normal * rays_d).sum(dim=-1))
           # print(converged)
            ...
            next_start_tmp = intersection - normal * 0.002
            
            intersections.append(intersection)
           
            bounding_box_index = torch.ones(rays_o.shape[0],device='cuda:0').int() * 9
           ...
           # print(bounding_box_index)
            ...
            ior_ratio = self.IORs.index_select(0,bounding_box_index)
            ior_ratio = torch.where(bounding_box_index == 9, 1, 1/1.5).reshape(-1,1)
            
          #  print(ior_ratio)
            if not is_outside: ior_ratio = 1 / ior_ratio
            ior_ratios.append(ior_ratio)
         #   print(ior_ratio * ior_ratio * sin_thetai_2)
            converged = torch.where(torch.logical_and(ior_ratio * ior_ratio * sin_thetai_2 > 1, converged), False, converged).detach()
           # print(converged)
            converges.append(converged)
            if torch.all(~converged): break

            sin_thetat_2 = torch.clamp(sin_thetai_2 * ior_ratio * ior_ratio,0,0.999)
         ...
         #   print(cos_thetai)
          #  print(sin_thetat_2)
            next_dir_tmp = (ior_ratio * next_dir + (ior_ratio * cos_thetai - torch.sqrt(1 - sin_thetat_2)) * normal)
          #  print(next_dir_tmp)
          #  next_dir_tmp[~converged.flatten()] = 1
        #    print(next_dir)
          #  print(next_dir_tmp)
           # exit(1)
            next_dir = torch.where(converged, next_dir_tmp,next_dir )
            # if i == 0:
            #     print(normal)
            #     print(next_dir)
            #     print(cos_thetai)
            ...
            #     print(sin_thetat_2)
            #     print(self.IORs)
            next_start = torch.where(converged, next_start_tmp, next_start)
       ...
       # print(bounding_box_index)
        ...
        # print('converges')
        # print(converges)
        converged_tmp = torch.ones_like(converges[0],device='cuda:0').bool()
        z_vals = torch.linspace(0, 1, 64).reshape(1,-1)  # sn
          # rn,sn
        sampled_pathes = []
        for k in range(len(converges)):
          #  print(converged_tmp.flatten().shape)
            
            start = starts[k][converged_tmp.flatten()].reshape(-1,3)
            #print(start.shape)
           # print(directions[i].shape)
            #print(intersections[i][converged_tmp.flatten()].shape)
            end = torch.where(converges[k][converged_tmp.flatten()], intersections[k][converged_tmp.flatten()],start + directions[k][converged_tmp.flatten()] * 8 ).reshape(-1,3)

            z_vals_outside = torch.linspace(1e-3, 1.0 - 1.0 / (32 + 1.0), 32)

            z_vals_outside = 1 / torch.flip(z_vals_outside, dims=[-1]) + 1.0 / 32

            z_vals_tmp = z_vals.repeat(start.shape[0],1)
            z_vals_tmp[~converges[k][converged_tmp.flatten()].flatten(),:32] *= 1.9
            z_vals_tmp[~converges[k][converged_tmp.flatten()].flatten(),32:] = z_vals_outside
           ...
           # print(((end - start) * z_vals.reshape(1,64,1)).shape)
            sampled_vertices = start.reshape(-1,1,3) + (end.reshape(-1,1,3) - start.reshape(-1,1,3)) * z_vals_tmp.reshape(-1,64,1)
      ...
      #  print(len(sampled_pathes))
        ...
        #trimesh.exchange.export.export_mesh(pc,'tst_pc5.ply')
        #print(sampled_pathes, converges, directions)
       # exit(1)
        #print(self.IORs)
       # print(directions)
        #exit(1)
        return sampled_pathes, converges, directions, ior_ratios
    ...
    def render_core(self, rays_o, rays_d, pathes, converges, directions, ior_ratios,human_poses, cos_anneal_ratio=0.0, step=None, is_train=True,
                    is_nerf=False):
        

        current_transmission_portion = torch.ones((converges[0].shape[0],3),device='cuda:0').reshape(-1,3)
        color = torch.zeros_like(rays_o,device='cuda:0')
        previous_coverges = torch.ones_like(converges[0],device='cuda:0').bool()
       ...
       # print(len(pathes))
         ...
         #   print(current_transmission_portion)
          ...
          #  print(i)
           ...
           # print(candidate_points.shape[0])
            ...
            #[N,3]
            candidate_directions = directions[i][previous_coverges.flatten()]
            
            #[N,1]
            candidate_converges = converges[i][previous_coverges.flatten()]
           # print(converges[i])
        ...
        #    print(human_poses.shape)
          #  human_poses_pt = human_poses.unsqueeze(-3).expand(points_for_neus.shape[0], points_for_neus.shape[1], 3, 4)
         #   dirs = F.normalize(dirs, dim=-1)
           ...
           # if torch.sum(outer_mask) > 0:
            ...
            bkgr_color = self.infinity_far_bkgr(directions[i].reshape(-1,3))

            # if torch.sum(inner_mask) > 0:
            #     alpha_nerf[inner_mask], sampled_color_nerf[inner_mask] = self.compute_density_alpha(points_for_nerf[inner_mask],
            #                                                                                 dists[inner_mask],
            #                                                                                 -dirs[inner_mask],
            #                                                                                 self.nerf_network)
            if points_for_neus.nelement() > 0:
                ...
                if i == 0:
                    #print('111')
                    # print(gradients)
                    # print((F.normalize(gradients.reshape(-1,3), dim=-1) + 1.0) * 0.5)

                    #  exit(1)
                    
                    normals_output[converges[i].flatten()] = (F.normalize(gradients.reshape(-1,3), dim=-1) + 1.0) * 0.5
            ...
            #print(feature_vector.shape)
                ...
                sampled_color_sdf, occ_info = self.color_network(points_for_neus, gradients, -dir_neus,
                                                                    feature_vector,ior_ratios[i][converges[i].flatten()].reshape(-1,1,1), i % 2 != 0,directions[i+1][converges[i].flatten()].reshape(-1,1,3), #, human_poses_pt,
                                                                    step=step)
            # Eikonal loss
            #     gradient_error = torch.zeros(1)
            # else:
          ...
          #  print(converges[i])
          #  print(color_nerf)
            color[previous_coverges.flatten()] += color_nerf * current_transmission_portion[previous_coverges.flatten()]
            transmission = torch.cumprod(torch.cat([torch.ones([batch_size, 1]), 1. - alpha_nerf + 1e-7], -1), -1)[:, -1:]
           # print(transmission.shape)
            current_transmission_portion[previous_coverges.flatten()] *= transmission
            color[previous_coverges.flatten() & ~converges[i].flatten()] += bkgr_color[previous_coverges.flatten() &~converges[i].flatten()] * current_transmission_portion[previous_coverges.flatten() &~converges[i].flatten()]
           # print(current_transmission_portion)
           # exit(1)
            #print(torch.any(torch.isnan(color_nerf)))
            # if(torch.any(torch.isnan(color_nerf))): 
            #     print('nerf nan' + str(i))
            #     print(sampled_color_nerf)
            #     print(weights_nerf)
            #     print(candidate_points)
            #     exit(1)
            #print(sampled_color_sdf.shape)
            if points_for_neus.nelement() > 0:
                color_neus = sampled_color_sdf[:,0,:]
                # print(torch.any(torch.isnan(color_neus)))
                # if(torch.any(torch.isnan(color_neus))): 
                #     print('neus nan')
                #     exit(1)
                #print(color_neus.shape)
               # try:
                color[converges[i].flatten()] += color_neus * current_transmission_portion[converges[i].flatten()]
                # except:
                #     print(i)
                #     print(converges)
                #     print(color_neus.shape)
                #     print(color[converges[i].flatten()].shape)       
                #     print(converges[i])
                #     print(candidate_converges)
                #     print(inner_mask)
                #     print(points_for_nerf)
                #     exit(1)

                current_transmission_portion[converges[i].flatten()] *= occ_info['refraction_coefficient'][:,0,:]
                previous_coverges = converges[i]
            else: break
            #print(converges[i].flatten())
          #  print(current_transmission_portion)
           # print(previous_coverges)
