...
class SphereTracing(torch.autograd.Function):
    ...
    def forward(
        ...
        *parameters,
        ...
        return ray_positions, converged & foreground_masks
    ...
    #       #  print(converged)
    #         grad_outputs = torch.where(converged, grad_outputs, torch.ones_like(grad_outputs) * 0.00001)
...
def sphere_tracing(
    ...
    if bounding_radius:
       ...
       # with torch.no_grad():
    for i in range(num_iterations):
        signed_distances = torch.clamp(signed_distance_function(ray_positions),-10,10)
        signed_distances = torch.nan_to_num(signed_distances, 10)
        
        if debug:
            if torch.any(torch.isnan(ray_positions)):
                print(ray_positions)
        ...
        converged = torch.abs(signed_distances) < convergence_threshold
        #if debug:
        #    print(signed_distances[(foreground_masks).flatten()])
        if torch.all(~foreground_masks | converged):
            break
    ...
    return ray_positions, (converged & foreground_masks).detach()
